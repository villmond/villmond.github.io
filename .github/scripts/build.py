...
def main(
    ...
    4. Copies logos and other static assets to the output directory
    ...
    # Copy static assets (e.g., logos) to the output directory
    static_assets = Path("static")
    if static_assets.exists():
        logger.info(f"Copying static assets from {static_assets} to {output_dir}")
        for asset in static_assets.glob("*"):
            if asset.is_file():
                target_path = output_dir / asset.name
                target_path.write_bytes(asset.read_bytes())
                logger.debug(f"Copied {asset} to {target_path}")
            else:
                logger.warning(f"Skipping non-file asset: {asset}")
